"""LP and sampling core for unweighted Correlated Sampling."""
...
def sample_arrival_sequence(I_size, p, T, use_poisson_len=False):
    """
    Generate an arrival sequence of original types from integerized e_i.
    """
...
def _require_scipy():
    if linprog is None or csr_matrix is None:
        raise ImportError(
            "The correlated_sampling algorithm requires scipy to solve its LP. "
...
def simulate_correlated_sampling_once(
    ...
    _e, T, p = _resolve_arrival_model(p, T)
