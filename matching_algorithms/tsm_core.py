"""TSM offline preprocessing and standalone reference helpers."""
...
from arrival_algorithms import (
    ...
    sample_arrival_sequence as sample_arrival_sequence_from_counts,
...
def compute_integer_eis(T, p):
    ...
    by giving +1 to the r largest fractional remainders.
    """
    return integerize_probability_vector(T, p)


def _resolve_arrival_counts(arrival_distribution, T=None):
    """
    New flow uses integer e_i. Old p,T callers are converted to e_i here.
    """
    if T is None:
        return validate_arrival_counts(arrival_distribution)
    return compute_integer_eis(T, arrival_distribution)
...
def simulate_tsm_once(A_size, I_size, neighbors, p, T,
    ...
    arrivals = sample_arrival_sequence_from_counts("fixed", I_size=I_size, e=e)
    for i in arrivals:
        imp_id = len(hat_I)
        hat_I.append(imp_id)
        imp_type.append(i)
...
def simulate_many_runs(A_size, I_size, neighbors, p, T=None, num_runs=1):
    """
    Main TSM wrapper:
        - receive integer e_i directly, or convert old p,T inputs to e_i
        - build expanded types and blue/red matchings
        - run multiple simulations, compute ALG/OPT ratio
    """
    # 1) resolve integer arrival distribution e_i
    e = _resolve_arrival_counts(p, T)
    T = sum(e)
    p_eff = arrival_probabilities_from_counts(e)
    ...
    for _ in range(num_runs):
        ALG, hat_I, edges_real, imp_type = simulate_tsm_once(
            A_size, I_size, neighbors, p_eff, T,
            blue_for_copy, red_for_copy, copies_of_type, e
        )
...
def random_arrival_distribution(I_size, T):
    """Random integer arrival distribution e_i with sum_i e_i = T."""
    return random_arrival_counts(I_size, T)
...
def simulate_many_runs_degree(A_size, I_size, neighbors, p, T=None, num_runs=1):
    e = _resolve_arrival_counts(p, T)
    T = sum(e)
    p_eff = arrival_probabilities_from_counts(e)
    ...
    for _ in range(num_runs):
        ALG, hat_I, edges_real, imp_type = simulate_degree_greedy_once(
            A_size, I_size, neighbors, p_eff, T, adv_degrees
        )
...
def simulate_random_graphs(
    ...
    for _ in range(num_graphs):
        ...
        e = random_arrival_distribution(I_size, T)

        avg_ratio, _ = simulate_many_runs(
            A_size, I_size, neighbors, e, num_runs=num_runs_per_graph
        )
