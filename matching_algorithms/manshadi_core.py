"""Shared offline-statistics primitives for the Manshadi variants."""
...
# ============================================================
# Standalone graph-generator compatibility helpers
# ============================================================
...
def _resolve_arrival_model(arrival_distribution, T=None):
    """
    New flow uses integer e_i. Old p,T callers are converted to e_i here.
    """
    if T is None:
        e = validate_arrival_counts(arrival_distribution)
    else:
        e = integerize_probability_vector(T, arrival_distribution)
    return e, sum(e), arrival_probabilities_from_counts(e)


# ============================================================
# Offline optimum on a realized bipartite graph
# ============================================================
...
def compute_opt_from_realization(A_size, hat_I, edges_real):
    ...
    Advertisers are counted on the left side of the maximum matching.
    """
    G = nx.Graph()
    left_nodes = []
...
def sample_arrival_sequence(I_size, p, T, use_poisson_len=False):
    """
    Generate arrival sequence of original types from integerized e_i.
    """
    e, _T, _p_eff = _resolve_arrival_model(p, T)
    mode = "poisson" if use_poisson_len else "fixed"
    return sample_arrival_sequence_from_counts(mode, I_size=I_size, e=e)
...
def estimate_f_monte_carlo(A_size, I_size, neighbors, p, T, mc_trials=200, seed=0, use_poisson_len=False):
    ...
    e, T, p = _resolve_arrival_model(p, T)
...
def simulate_many_runs_offline_statistics(
    ...
    """
    Standalone wrapper for repeated offline-statistics simulations:
