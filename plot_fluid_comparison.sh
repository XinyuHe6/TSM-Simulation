...
ALGORITHMS="fluid,random_matching,degree_matching,tsm,manshadi2,manshadi3,manshadi4"

SEED=0

ALG_TAG="${ALGORITHMS//,/+}"
OUT_DIR="out_fluid_comparison_A${A}_I${I}_T${T}_E${EDGE_POINTS}_G${NUM_GRAPHS_PER_POINT}_R${RUNS_PER_GRAPH}_MC${MC_TRIALS}_${ALG_TAG}"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" plot_fluid_comparison.py \
  --A "$A" \
  --I "$I" \
  --T "$T" \
